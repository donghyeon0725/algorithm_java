...
* 너무 느린 입출력 방식 선택
    - 자바의 경우 scanner이냐 bufferReader 이냐에 따라 속도가 많이 차이남
    - scanner 는 대략 1MB 인데 반해 bufferedreader 는 8MB 이다.
    - 게다가, Scanner 에는 내부적으로 문자열을 구분하는 파싱 기능이 있지만 bufferedreader 에는 없어서 상대 속도가 빠르다
* 변수 초기화 문제
...
📌 소수에 대해 더 알아보려면 
-
[소수](../dataType/flo/README.md "")

<br/>

📌 코드의 수치적 안정성 파악하기 
-
* 수치적으로 안정적인 프로그램을 짜려면 되도록 실수 연산을 사용하지 않는 방법이 있음
    - 곱셈과 나눗셈의 순서를 바꾸기
        - a/b*c => (a*c)/b
    - 양변 제곱하기
        - 거리가 맞는지 확인만 할때 a^2 + b^2 에 루트를 띄울 것이 아니라, 양변을 제곱해서 식이 성립하는지만 보면 됨
...
📌 알고리즘이란?
-
* 주어진 문제를 해결하기 위한 **명료한** 방법
    1. 지하철 2호선을 타고 시청역으로 가기 
    2. 지하철 1호선으로 갈아타고 청량리역으로 가기
    3. 경춘선을 타고, 춘천역에서 내린다.
...
-
* 기준
    - 반복문 : 입력 값이 무수히 커질 때 주요한 시간을 차지하는 척도가 되므로 

* 예시
    - 입력값 N이 있을 때, N + 100 번 작업하는 알고리즘의 시간복잡도는 N
    - 이유는 N이 무수히 커질 때 100이 값에 영향을 미치는 정도는 점점 작아지므로
...
-
* 선형 시간 알고리즘
    - 입력값 N에 대해서 비례해서 일정하게 시간 복잡도가 증가하는 알고리즘 => O(n)
* 선형 이하 알고리즘
    - 입력한 N에 대해, 시간 복잡도의 "증가하는 정도가" 점점 감소하거나, 증가하지 않는 알고리즘 => O(logn), O(1)
* 지수 시간 알고리즘
    - 다항 시간 알고리즘, 지수시간 알고리즘
        - 다항 시간 알고리즘 : N^2, N^3, N^4 .... 
        - 지수시간 알고리즘 : 다항 시간 알고리즘 보다 훨씬 더 느린 알고리즘. 2^N (입력값이 지수값이 되어버리는 경우)
...
-
* 이동 평균을 구하는 알고리즘
    - 이동 평균이란, 주식 수치와 같이 일정 기간의 값 평균을 낸 값을 모은 것이다.
    - 예를 들어서 배열 [3, 7, 5, 2, 1] 값이 있다고 할 때 3이동 평균은 [5, 4.666666, 2.6666] 으로 3개의 값이 나온다.
        * 방법 1. 모든 값을 반복해서 3으로 나눈 평균을 구한다.
        * 방법 2. 이동 평균의 첫 값을 구한 뒤, 다음 값 부터는 맨 처음 값을 이동평균에서 제외시키고 새로 들어온 값만을 포함 시키는 방법 (중복된 이전의 값들을 다시 재사용함)
        * 방법 1의 경우 O(n^2) 이지만 방법 2번의 경우 O(n) 
        * 입력 크기 대비 걸리는 시간을 그래프로 그려보면 직선이 되기 때문에 이런 알고리즘을 선형 알고리즘이라고 부름
        * 보통의 경우 선형 시간 알고리즘은 좋은 알고리즘이라고 부름 

* 선형 이하 시간 알고리즘
    - 정렬된 자료의 경우 반반씩 지점을 정해서 값을 찾으면 되므로 이때 시간복잡도는 O(logn) (밑은 2)
    - 이런 방식을 이진 탐색이라고 부름

* 다항 시간 알고리즘
    - 다항식 N^2, N^3 등등 차수가 있는 식으로 표현이 가능하면 다항 시간 알고리즘이라고 부름
    - 대다수의 알고리즘이 여기에 포함 됨

* 지수 시간 알고리즘
    - 보통 모든 경우의 수를 다 따져와야할 때 지수시간 알고리즘이라고 함
    - 예를 들어서 음식 a,b,c,d가 있고 사람 A,B,C,D 가 있다고 하고 각각 먹을 수 있는 음식의 종류가 다를 때 음식을 준비하려면 우리는
        - a, b, c, d, (a,b), (a,c), (a,d), .... 이런식으로 모든 경우의 수를 다 따져봐야 함
        - 이를 트리모양으로 쭉 나열할 수 있는데, 이럴 때 재귀호출이 많이 사용됨.
        - 이는 모든 후보를 검사하고 가장 좋은 답을 반환하는 방법임
        - 앞서 말한 예시의 시간복잡도는 순서는 상관이 없고, 오직 각각의 메뉴에 대해 넣을지 말지만 결정하면 되므로 2^N 즉 O(2^n)이다.
    - 또하나 예를 들어서 소인수 분해의 경우 (소수인 약수 구하기)
        * 1부터 입력 n까지에 해당 하는 모든 수를 나누어 보면서 소수(나누어지는지)인지 판별하는 로직이므로 n이 소수인 경우 최대 n번을 반복하므로, 선형 시간 알고리즘이라고 생각하기 쉬우나 숫자 자체가 반복 회수에 직접적인 영향을 미침. 그래서 숫자를 기준으로 보려고 하면 특정 기준을 낼수가 없음. 
        * 특정 수를 기준으로 하자니 1만 달려져도 시간 복잡도 그래프가 요동침, 다만 입력을 숫자가 아닌 비트수로 보자면, 비트가 하나 증가할 때마다, 표현할 수 있는 수의 범위는 2배 늘어나고, 입력 값으로 넣을 수 있는 값이 두배 늘어난다고 의미할 수 있음. 그래서 시간 복잡도는 지수 시간이다.
        * 이와 관련해서 하나의 글을 보자
        
        
        ```html
        https://gall.dcinside.com/board/view/?id=mathematics&no=335416&t=cv&page=1
        입력값을 시간이 아닌, 데이터 크기로 봄에 따라서 다르게 해석할 수 있다.
                
        다항시간을 따질때는 데이터의 크기로 따지는데 현대의 컴퓨터에서 n이란 수를 표현할 때는 2진법을 쓰기 때문에 n의 데이터로서의 크기는 log_2 n 이 됨.
        "데이터 크기"가 n일때 시간이 n만큼 걸리는 알고리즘은 f(n)=n 이 n에 대한 다항식이니까 다항시간이지만, 데이터 크기가 log_2 n일때 시간이 n만큼 걸리는 알고리즘은 f(log_2 n)=n, 즉 f(x)=2^x 라서 다항시간이 아니라 지수시간 이 됨
        ```
...
* 기본적인 연산의 수를 입력의 크기에 대한 함수로 표현한 것
    - 여기서 기본적인 연산이란? 더 쪼갤 수 없는 최소 크기의 연선
        - 사칙연산
        - 대소비교
        - 변수대입
    - 반면 기본적인 연산이 아닌 것은 => 반복하는 것
        - 정수 배열 정렬
        - 같은 문자인지 확인
        - 소인수 분해하기
    - => 기본적인 연산이 시간 복잡도의 대략적인 기준이 된다.
...
📌 추가 정보
-
* 시간 복잡도가 낮다고 해서 무조건 빠른 알고리즘은 아님. 입력값이 충분히 낮을 때는 때때로 시간복잡도가 더 높은 알고리즘이 더 빠를때가 있음
